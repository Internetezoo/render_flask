...
def decode_jwt_device_id(token: str) -> Optional[str]:
    """Kinyeri a device_id-t a JWT token payload részéből."""
    try:
        parts = token.split('.')
...
def call_content_api(content_id, token, device_id, season_num):
    """A feltöltött fájlod alapján összeállított precíz API hívás."""
    
    # Ha a kliens None-t küldött, kihalásszuk a tokenből
    ...
    # A beküldött fájlod alapján kiegészített paraméterek
    params = {
        "app_id": "tubitv",
        "platform": "web",
    ...
    logging.info(f"🔗 Precíz API hívás: ID {content_id}, Season {season_num}")
...
async def scrape_auth_and_html(url: str):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logging.info("⏳ Várakozás a tokenre (5mp)...")
...
def main():
    ...
    # Kliens küldi vissza a második körben
    ...
    html_content = ""
    # 1. KÖR: Ha nincs token, fut a Playwright
    if not token:
        logging.info("🕵️ Token kinyerése folyamatban...")
    ...
    # 2. KÖR: Ha van token és évad, hívjuk az API-t
