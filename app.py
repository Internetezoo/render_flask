...
MAX_RETRIES = 3
# <<< JAVÍTOTT KONSTANS: Átállítva a standard Tubi fejléc-névre a megbízható Device ID kinyeréshez
DEVICE_ID_HEADER = "X-Tubi-Client-ID" 
...
        return None
        
# --- API HÍVÁS PAGINÁLT ÉVADHOZ (Változatlan a logikában, de a fejléceket a korábbi javítás miatt most már a helyes konstanst használja) ---
...
        DEVICE_ID_HEADER: device_id, # <<< JAVÍTOTT KONSTANS HASZNÁLATA
...
# --- RÉGI/DEFAULT API HÍVÁS (Csak az S1 metaadatokhoz - szintén a javított konstanst használja) ---
...
    """Várakozik a 'tubi_token' és 'tubi_device_id' megjelenésére a 'results' szótárban, polling módszerrel."""
    start_time = time.time()
    
    while time.time() - start_time < timeout:
        # <<< JAVÍTÁS: Mind a token, mind a device_id meglétét ellenőrizzük
        if results.get('tubi_token') and results.get('tubi_device_id'):
            return True
...
# ----------------------------------------------------------------------
# ASZINKRON PLAYWRIGHT SCRAPE FÜGGVÉNY (A token kinyerés beágyazott része, változás nélkül)
# ----------------------------------------------------------------------
...
                        
                        # <<< Itt a konstans átvált a DEVICE_ID_HEADER: "X-Tubi-Client-ID" értékre
...
                # <<< JAVÍTOTT VÁRAKOZÁS: A wait_for_token most már a Device ID-re is vár
                token_found = await wait_for_token(results, timeout=15)
                
                if token_found:
                    logging.info("🔑 Token és Device ID sikeresen rögzítve a várakozási ciklusban.")
                elif not results.get('tubi_token'):
                    logging.warning("❌ A token nem került rögzítésre a 15 másodperces várakozási időn belül.")
                elif not results.get('tubi_device_id'):
                    logging.warning("❌ A Device ID nem került rögzítésre a 15 másodperces várakozási időn belül. Próba a JWT dekódolással (Fallback 2).")
...
        
        # Belső API hívás
        if target_api_enabled and token_present and device_id_present and not api_data_present:
            content_id = extract_content_id_from_url(url)
...
                user_agent=final_data.get('user_agent', 'Mozilla/5.0')
            )
            if final_data['tubi_api_data'] is None:
                final_data['status'] = 'partial_success'
                final_data['error'] = final_data.get('error', 'Token rendben, de az S1 metaadat API hívás sikertelen volt.')
            
        if final_data.get('status') == 'success' and (not target_api_enabled or (token_present and final_data.get('tubi_api_data') is not None)):
              return jsonify(final_data)
        
