...
def extract_content_id_from_url(url: str) -> Optional[str]:
    ...
    for segment in reversed(path_segments):
        # HIBA JAVÍTVA: A behúzás most már helyes
        if segment.isdigit():
            return segment
    return None
...
def make_paginated_tubi_api_call(
    ...
    for page_num in range(1, max_pages + 1):
        ...
        # HIBA JAVÍTVA: A try blokk behúzása helyes
        try:
            ...
            collected_page_data.append({
                ...
                # HIBA JAVÍTVA: A page_size kulcs behúzása helyes
                "page_size": page_size,
...
def make_internal_tubi_api_call(api_type: str, url: str, content_id: Optional[str], token: str, device_id: str, user_agent: str) -> Optional[Dict]:
    ...
    if api_type == 'content':
        if not content_id:
            # HIBA JAVÍTVA: A logging.error behúzása helyes
...
async def scrape_tubitv(url: str, target_api_enabled: bool, har_enabled: bool, simple_log_enabled: bool, api_type: str) -> Dict: 
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # HIBA JAVÍTVA: Az await temp_context.close() behúzása helyes
            ...
            # HIBA JAVÍTVA: Az await page.goto behúzása helyes
            ...
            if target_api_enabled:
                ...
                while not results.get('tubi_token') and (time.time() - start_time) < MAX_POLL_TIME:
                    ...
                    if results.get('tubi_token'):
                        ...
                        # HIBA JAVÍTVA: A break behúzása helyes
                        break
...
def scrape_tubi_endpoint():
    ...
    for attempt in range(1, retry_count + 1):
        ...
        # Minden return utasítást 4 szóköznyi behúzásra állítottam a környező if blokkokon belül
